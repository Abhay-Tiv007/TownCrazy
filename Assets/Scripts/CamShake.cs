...
public class CamShake : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!camShakeAcive)
        {
            ...
            Vector3 newPos = Vector3.Lerp(transform.localPosition, Vector3.zero, Time.deltaTime*10);
    ...
    private IEnumerator Shake(float magnitude, float duration)
    {
        ...
        while (elapsedTime <= duration) {
            ...
            if (Trauma > 0)
            {
                ...
                Vector3 newPos = traumaMag * Trauma * GetVec3() ;
                transform.localPosition = newPos;
    ...
    {
        StartCoroutine(Shake(0.5f, duration));
    }
